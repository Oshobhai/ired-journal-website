...
export default async function SearchPage({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  return <><Header/>
    ...
    <main className="container" style={{padding:'22px 0 34px'}}>
      <form action="/search" method="get" style={{display:'flex',gap:8,maxWidth:760,marginBottom:18}}>
      ...
      {!term?<div className="contentCard"><p style={{margin:0,color:'#687586'}}>Enter a search term above.</p></div>:<div className="contentCard">
        <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',marginBottom:12}}><h2 style={{margin:0}}>Results for “{term}”</h2><span style={{fontSize:11,color:'#71808d'}}>{results.length} result(s)</span></div>
        {results.length?results.map(r=><div key={`${r.kind}-${r.id}`} style={{display:'grid',gridTemplateColumns:'84px 1fr auto',gap:14,alignItems:'center',padding:'13px 0',borderTop:'1px solid #e3e8ec'}}>
