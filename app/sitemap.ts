...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  const staticPaths=['','/about','/green','/red','/editorial-board','/author-guidelines','/publication-ethics','/archives','/contact','/privacy','/terms']
  ...
  return [
    ...staticPaths.map((path,index)=>({url:`${base}${path}`,changeFrequency:(index===0?'weekly':'monthly') as 'weekly'|'monthly',priority:index===0?1:0.7})),
